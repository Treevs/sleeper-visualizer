...
function App() {
    ...
    const LEAGUE_ID_2024 = "1117345864621723648";
    ...
    const [medians2024, setMedians2024] = useState([]);
    ...
    const [matchups2024, setMatchups2024] = useState([]);
    ...
    const mapData = async () => {
        const {users, rosters} = await getUsersAndRosters(LEAGUE_ID_2024);
        ...
        await setMatchups2024(await getMatchups(LEAGUE_ID_2024));
        setMappedUsers(mapUsers({users, rosters, matchups: matchups2024}));
    }

    const calculateTreevors = () => {
        //find second highest score
        const matchups = matchups2021.concat(matchups2022).concat(matchups2023).concat(matchups2024)
    ...
    useEffect(() => {
        if(users.length > 0 && rosters.length > 0 && matchups2024.length > 0) {
            setMappedUsers(mapUsers({users, rosters, matchups: matchups2024}));
        }
    }, [matchups2024])
    useEffect(() => {
        ...
        setMedians2024(calculateMedians(matchups2024));
        if(mappedUsers.length > 0) {
            calculateTreevors()
        }
    }, [mappedUsers, matchups2021, matchups2022, matchups2023, matchups2024])
    ...
    return (
        <div className="App">
            <div className="content">
                ...
                <MedianTable medians2021={medians2021} medians2022={medians2022} medians2023={medians2023} medians2024={medians2024}/>
